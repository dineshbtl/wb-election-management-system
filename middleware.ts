...
export function middleware(req: NextRequest) {
  ...
  // 1️⃣ LOGIN REQUIRED (but no role restrictions for /surveys)
  if (
    !token &&
    (url.startsWith("/dashboard") ||
      url.startsWith("/analytics") ||
      ...
      url.startsWith("/test") ||
      url.startsWith("/reports") ||
      url.startsWith("/surveys") || // 👈 surveys needs *token only*
      url.startsWith("/team") ||
      url.startsWith("/cameras") ||
      url.startsWith("/locations") ||
      url.startsWith("/map") ||
      ...
      url.startsWith("/district") ||
      url.startsWith("/assembly") ||
      url.startsWith("/block") ||
      url.startsWith("/booth") ||
  ...
  // 2️⃣ SUPERADMIN ROUTES (surveys removed)
  if (
    (url.startsWith("/dashboard") ||
      url.startsWith("/analytics") ||
      url.startsWith("/products") ||
      url.startsWith("/test") ||
      url.startsWith("/reports") ||
      // ❌ removed /surveys from superadmin restriction
      url.startsWith("/cameras") ||
      url.startsWith("/locations") ||
      url.startsWith("/map") ||
      url.startsWith("/boq") ||
  ...
  // 3️⃣ Other role-specific protections (unchanged)
  if (url.startsWith("/district") && role !== "district coordinator") {
    return NextResponse.redirect(new URL("/", req.url));
  }
