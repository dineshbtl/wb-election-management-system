...
import { Input, Select, Upload, message, Spin, Skeleton } from "antd";
...
export default function BlockCoordinatorPage() {
  ...
  useEffect(() => {
    if (token1) {
      const initializeData = async () => {
        ...
        try {
          await Promise.all([fetchUser(), fetchDistricts(), fetchRoles()]);
        } finally {
  ...
  const fetchRoles = async () => {
    try {
      ...
      const allowedRoleNames = ["Assembly coordinator", "Booth coordinator"];

      const filteredRoles = (res.data.roles || []).filter((role: any) =>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (form.Photo) {
        ...
        formData.append(
          "refId",
          appUserRes.data?.data?.id || appUserRes.data?.id,
        );
