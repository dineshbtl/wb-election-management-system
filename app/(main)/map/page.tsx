...
export default function MapPage() {
  ...
  // 🔹 Fetch all projects with sites
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const res = await api.get("/projects?populate=site_1s");
        const projectData = res.data?.data || [];
        setProjects(projectData);

        // Collect all sites initially
        const allSites = projectData.flatMap((p: any) =>
          (p.site_1s || []).map((s: any) => ({
            ...s,
  ...
  const handleNavigate = (lat: number, lng: number) => {
    ...
    // If user location available, include it
  ...
  const initializeMap = useCallback(() => {
    ...
    const map = new window.google.maps.Map(mapRef.current, {
      ...
      zoom: 7,
      center: { lat: 15.9129, lng: 79.74 }, // Center on India by default
    });
      ...
      const infoWindow = new window.google.maps.InfoWindow({
        content: `
            <div style="max-width: 250px;">
              <h3 style="font-weight:bold;color:#111;">${site.name}</h3>
  ...
  // 🔹 Add markers when filteredSites change
