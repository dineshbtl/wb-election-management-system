  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        const res = await api.get("/users/me?populate[profile][populate]=Photo");
        ...
        // If profile not populated (null/empty), try to fetch app-user linked to this user
        let profile = userData.profile ?? null;
        if (!profile || (typeof profile === "object" && !profile.documentId && !profile.id)) {
          const userId = userData.documentId ?? userData.id;
          if (userId) {
            try {
              const appRes = await api.get(
                `/app-users?filters[user][documentId][$eq]=${userId}&populate[Photo]=true`
              );
              const raw = appRes.data?.data ?? appRes.data;
              const arr = Array.isArray(raw)
                ? raw
                : raw && (raw.id != null || raw.documentId != null)
                  ? [raw]
                  : [];
              if (arr.length > 0) profile = arr[0];
            } catch {
              try {
                const appRes = await api.get(
                  `/app-users?filters[user][id][$eq]=${userData.id}&populate[Photo]=true`
                );
                const raw = appRes.data?.data ?? appRes.data;
                const arr = Array.isArray(raw)
                  ? raw
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-blue-50 to-green-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8 max-w-md w-full border-l-4 border-red-500">
          <div className="flex items-center gap-3 mb-4">
            <AlertCircle className="w-8 h-8 text-red-500" />
            <h3 className="text-base sm:text-base sm:text-base sm:text-lg font-bold text-gray-900">
              Error
            </h3>
          </div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-6 sm:p-8 border-l-4 border-orange-500">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                {/* Change Password hidden for all users as of now */}
                <Button
                  type="primary"
                  size="large"
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
              ...
              <div className="space-y-4">
                {/* Row 1 */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 gap-4">
                  <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
                    <p className="text-sm text-gray-600 font-medium mb-1">
                      Full Name
                    </p>
                    <p className="text-base sm:text-base sm:text-base sm:text-lg font-semibold text-black">
                      {profile.Full_Name || "—"}
                    </p>
                  </div>
                  <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
                    ...
                    <p className="text-base sm:text-base sm:text-base sm:text-lg font-semibold text-black flex items-center gap-2">
                ...
                <div className="bg-gradient-to-r from-blue-100 to-green-100 p-4 rounded-lg border-2 border-blue-200">
                  ...
                  <p className="text-lg sm:text-xl font-bold text-black break-all">
